...
end_date <- Sys.Date() # as.Date("2015-12-31")
extensao_DB <- ".zoo"

verbose <- T # FALSE #TRUE

### Constantes do livro ###
from <- start_date
to <- end_date
initDate <- "1998-01-01" #by HH "1990-01-01"

# casa:
# setwd("C:/user/hlobe/Documents/Recherches/Treading/R for traders - (Book- Quant trading with R)/Chapter_07")
# boulot:
# setwd("D:/HL/Dropbox (LaMaison)/Recherche geral/Treading/R for traders - (Book- Quant trading with R)/Chapter_07")

# carrega a Utils.R
source("utils-livro.R")

# General environment setup
cat("Setting the environment...\n") # show a message in the console
# list of package for easy maintenance #
list.of.packages <- c(
  "utils",
  "here",
  "zoo",  #  already call by quantmod
  "xts",  #  already call by quantmod
  "quantmod",
  "TTR",
  "blotter",
  "quantstrat",
...
# Load packages
load_package(verbose, list.of.packages)
...
# ja setado pelo projeto. OLD: setwd(Path_FTSE) # seta o working directory
cat("* usando o diretorio:", getwd(), "\n")
cat("\n\t*** TUDO PRONTO! ***", format(Sys.time(), "%c"), "\n")
...
# Suppresses warnings
# by HH options("getSymbols.warning4.0" = FALSE)

print_hh("Carregando os dados...")
...
"osDollarATR" <- function(orderside, tradeSize, pctATR, maxPctATR = pctATR, data, timestamp,
                          symbol, prefer = "Open", portfolio, integerQty = TRUE, atrMod = "",
                          rebal = FALSE, ...) {
  ...
  if (is.na(atrTimeStamp) | atrTimeStamp == 0) {
    stop(
      paste(
        "ATR corresponding to",
        atrString,
        "is invalid at this point in time.  Add a logical operator to account for this."
      )
    )
  }
...
#by HH require(quantstrat)
#by HH require(PerformanceAnalytics)

#by HH initDate <- "1990-01-01"
#by HH from <- "2003-01-01"
#by HH to <- "2013-12-31"
options(width = 70)

# To rerun the strategy, rerun everything below this line
# demoData.R contains all of the data-related boilerplate.
#by HH source("demoData.R")

# Trade sizing and initial equity settings ####
###
tradeSize <- 10000 # in USD? by HH
initEq <- tradeSize * length(symbols) # total money in USD? by HH
...
print_hh( verbose, head(round(test, 2), 5))
print_hh( verbose, round(test[c(nLag:(nLag+5)),], 2))


### by HH
if(verbose == T) 
  if(askYesNo("Stopa o codigo?")==T) stop("Parado pelo usuario (by HH)")
...
#by HH dando erro: applyStrategy()
...
###
print_hh("Resumo da performance")
...
tStats <- tradeStats(
  Portfolios = portfolio.st, use = "trades",
  inclZeroDays = FALSE
)
...
meanAvgWLR <- mean(tStats$Avg.WinLoss.Ratio[
  tStats$Avg.WinLoss.Ratio < Inf
], na.rm = TRUE)

print_hh(verbose, "tStats")
cat_hh(verbose, "\n\n-----x-----\n")
cat("Aggregate win-to-Loss ratio: ", format(aggPF, digits = 2, nsmall = 2), "\n")
cat("Aggregate win rate: ", format(aggCorrect, digits = 3, nsmall = 2), "%\n")
cat("Aggregate number of trades: ", numTrades, "\n")
cat("Average win-to-loss ratio: ", format(meanAvgWLR, digits = 2, nsmall = 2), "\n")
...
# Obtain symbol
getSymbols("SPY", from = firstNonZeroDay, to = to, auto.assign = TRUE)
SPYrets <- diff(log(Cl(SPY)))[-1]
cumSPYrets <- cumprod(1 + SPYrets)
comparison <- cbind(cumPortfRets[paste0(start(cumSPYrets),"/")], cumSPYrets)
colnames(comparison) <- c("strategy", "SPY")
#chart.TimeSeries( comparison,
#  legend.loc = "topleft",
#  colors = c("green", "red")
#)
chart.RelativePerformance(cumPortfRets[paste0(start(cumSPYrets),"/2012-01-01")],
                          cumSPYrets["/2012-01-01"],
                          legend.loc = "topleft",
                          colors = c("green", "red")
                          )
...
connorsRSI <- function(price, nRSI = 3, nStreak = 2,
  ...
  percents <- round(runPercentRank(
    x = diff(log(price)),
    n = 100, cumulative = FALSE, exact.multiplier = 1
  ) * 100)
...
add.indicator(strategy.st,
  name = "cumCRSI",
  arguments = list(
    price = quote(Cl(mktdata)), nCum = nCum,
...
add.indicator(strategy.st,
  name = "connorsRSI",
  arguments = list(
    price = quote(Cl(mktdata)), nRSI = nRSI,
    nStreak = nStreak,
    nPercentLookBack = nPercentLookBack
  ),
  label = "CRSI"
...
durationStatistics <- function(Portfolio, Symbols,
  ...
  for (Symbol in Symbols) {
    ...
    dataRow <- data.frame(cbind(
      t(round(durationSummary)),
